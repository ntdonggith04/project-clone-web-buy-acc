...
function coverEyes() {
	...
	gsap.to(armL, .45, {x: -93, y: 10, rotation: 0, ease: Quad.easeOut});
	gsap.to(armR, .45, {x: -93, y: 10, rotation: 0, ease: Quad.easeOut, delay: .1});
	gsap.to(bodyBG, .45, {morphSVG: bodyBGchanged, ease: Quad.easeOut});
	eyesCovered = true;
}
...
function uncoverEyes() {
	...
	gsap.to(armR, 1.35, {rotation: -105, ease: Quad.easeOut, delay: .1, onComplete: function() {
		gsap.set([armL, armR], {visibility: "hidden"});
	}});
	gsap.to(bodyBG, .45, {morphSVG: bodyBG, ease: Quad.easeOut});
	eyesCovered = false;
}
...
function startBlinking(delay) {
	...
	blinking = gsap.to([eyeL, eyeR], .1, {delay: delay, scaleY: 0, yoyo: true, repeat: 1, transformOrigin: "center center", onComplete: function() {
		startBlinking(12);
	}});
}
